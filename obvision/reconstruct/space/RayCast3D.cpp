...
void RayCast3D::calcCoordsFromCurrentPose(TsdSpace* space, Sensor* sensor, double* coords, double* normals, unsigned char* rgb, unsigned int* size)
{
  ...
  *size = 0;
  ...
  std::cout << __PRETTY_FUNCTION__ << "sensor->getWidth() =" << sensor->getWidth() << std::endl;
  std::cout << __PRETTY_FUNCTION__ << "sensor->getHeight() =" << sensor->getHeight() << std::endl;
  std::cout << __PRETTY_FUNCTION__ << "sensor->getMinimumRange =" << sensor->getMinimumRange() << std::endl;
  std::cout << __PRETTY_FUNCTION__ << "sensor->getMaximumRange() =" << sensor->getMaximumRange() << std::endl;
  for(int i = 0; i < 10; i++)
  {
    std::cout << (*R)(0, i) << " " << (*R)(1, i) << " " << (*R)(2, i) << std::endl;
  }
...
void RayCast3D::calcCoordsFromCurrentPoseMask(TsdSpace* space, Sensor* sensor, double* coords, double* normals, unsigned char* rgb, bool* mask,
                                              unsigned int* size)
{
  Timer t;
  t.start();
...
/*bool RayCast3D::calcCoordsFromCurrentPose(TsdSpace* space, Sensor* sensor, double* coords, double* normals, unsigned char* rgb, const
std::vector<TsdSpace*>& spaces,
...
bool RayCast3D::rayCastFromSensorPose(TsdSpace* space, obfloat pos[3], obfloat ray[3], obfloat coordinates[3], obfloat normal[3], unsigned char rgb[3],
                                      obfloat* depth)
{
  obfloat position[3];
