                                             ...
                                             Sensor(3, maxRange, minRange, lowReflectivityRange)
{
  ...
  for(unsigned int i = 0; i < raysAzim; i++)
  {
    ...
    {
      //to follow definition of theta (inclination angle) in spherical coordinate system
      ...
      //normalize rays
      const double length     = sqrt(calcRays(0,0)*calcRays(0,0)+calcRays(1,0)*calcRays(1,0)+calcRays(2,0)*calcRays(2,0));
...
{
  //Inclination
  double theta = 0.0;   //careful, this is angle between z-axis and x-y-plane as defined in polar coordinates --> no distinction of cases for acos necessary, bec. only values from 75° - 105° for VLP16
  ...
  {
    if(theta > 90.0)  //translate theta into inclination angle "aperture angle" from -15° to +15°
    {
...
void SensorVelodyne3D::returnRayIndex(double azimAngle, double inclAngle, unsigned int* azimIndex, unsigned int* inclIndex)
{
  *azimIndex = round(azimAngle/_azimRes);
  ...
  double mapInclination = inclAngle + 15.0;       //map inclination angles (-15° -> +15°) up to positive range 0° - 30° --> change so this also works for E32
...
unsigned int SensorVelodyne3D::lookupIndex(int indexSensormodel)
{
  ...
  {
  case 0: indexVelodyneROS = 0;
  break;
  case 1: indexVelodyneROS = 2;
  break;
  case 2: indexVelodyneROS = 4;
  break;
  case 3: indexVelodyneROS = 6;
  break;
  case 4: indexVelodyneROS = 8;
  break;
  case 5: indexVelodyneROS = 10;
  break;
  case 6: indexVelodyneROS = 12;
  break;
  case 7: indexVelodyneROS = 14;
  break;
  case 8: indexVelodyneROS = 1;
  break;
  case 9: indexVelodyneROS = 3;
  break;
  case 10: indexVelodyneROS = 5;
  break;
  case 11: indexVelodyneROS = 7;
  break;
  case 12: indexVelodyneROS = 9;
  break;
  case 13: indexVelodyneROS = 11;
  break;
  case 14: indexVelodyneROS = 13;
  break;
  case 15: indexVelodyneROS = 15;
  break;
  }
